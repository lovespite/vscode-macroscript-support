...
export function activate(context: vscode.ExtensionContext) {
    ...
    const provider = vscode.languages.registerCompletionItemProvider('macroscript', {
        ...
        provideCompletionItems(
            ...
            // 2. 鼠标按钮补全
            const mouseButtonCompletions = mouseButtons.map(val =>
            ...
            namedScriptSnippet.insertText = new vscode.SnippetString(
                'Script(${1:`\n\n`}, ${2:"MyScript"})'
            );

            const unnamedScriptSnippet = new vscode.CompletionItem('Script(content)', vscode.CompletionItemKind.Snippet);
            unnamedScriptSnippet.insertText = new vscode.SnippetString(
                'Script(${1:`\n\n`})'
            );
