...
function activate(context) {
    const provider = vscode.languages.registerCompletionItemProvider('macroscript', {
        provideCompletionItems(document, position, token, context) {
            ...
            // 2. 枚举值补全
            const mouseActionCompletions = mouseActions.map(val => new vscode.CompletionItem(val, vscode.CompletionItemKind.EnumMember));
            const keyActionCompletions = keyActions.map(val => new vscode.CompletionItem(val, vscode.CompletionItemKind.EnumMember));
